...
export default function DealOfTheWeek({ products }) {
  if (products.length === 0) return null;
  return (
    // <div>
    //   <div className="bg-secondary w-full flex justify-center items-center text-lg uppercase tracking-wide mb-8 px-4 h-11 ">
    //     <div>SẢN PHẨM ĐANG HOT</div>
    //   </div>
    <marquee scrollAmount={18} >
      <ul className="grid xxs:grid-cols-2 gap-4  mx-auto sm:grid-cols-5col-280 md:gap-6 w-full lg:gap-12">
        {products.map((item) =>
          item.is_publish ? (
            <li key={item.id}>
